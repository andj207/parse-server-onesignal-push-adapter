...
var OneSignalPushAdapter = exports.OneSignalPushAdapter = function () {
    function OneSignalPushAdapter() {
        ...
        var configSize = Object.keys(pushConfig).length;
        if (configSize == 0) {
    ...
    _createClass(OneSignalPushAdapter, [{
        ...
        value: function send(data, installations) {
            ...
            for (var pushType in deviceMap) {
                ...
                if (devices.length > 0) {
                    var devicesByApp = _.groupBy(devices, function (i) {
                        return i['appIdentifier'];
                    });
                    for (var appIdentifier in devicesByApp) {
                        var config = this.OneSignalConfig[appIdentifier];
                        if (config) {
        ...
        key: 'sendToAPNS',
        value: function sendToAPNS(data, tokens, config) {
        ...
        key: 'sendToGCM',
        value: function sendToGCM(data, tokens, config) {
            data = deepcopy(data['data']);
        ...
        key: 'sendToOneSignal',
        value: function sendToOneSignal(data, config, cb) {
            var headers = {
